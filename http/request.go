...
func (r Request) ReadJSON(obj interface{}) error {
	...
	if err != nil {
		return errors.Wrap(err, "could not read JSON data from request body")
	}
	...
	if err != nil {
		return errors.Wrap(err, "could not unmarshal JSON data from request body")
	}
...
func (r Request) ReadXML(obj interface{}) error {
	buf, err := ioutil.ReadAll(r.Request.Body)
	if err != nil {
		return errors.Wrap(err, "could not read XML data from request body")
	}
	...
	if err != nil {
		return errors.Wrap(err, "could not unmarshal XML data from request body")
	}
...
func (r Request) ReadString() (string, error) {
	buf, err := ioutil.ReadAll(r.Request.Body)
	if err != nil {
		return "", errors.Wrap(err, "could not read string data from request body")
	}
